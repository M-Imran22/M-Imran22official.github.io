  ...
  while (remProcesses.length > 0) {
    ...
    for (let i = 0; i < remProcesses.length; i++) {
      if (
        ...
        remProcesses[i].burstTime < shortestBurstTime
      ) {
        shortestBurstTime = remProcesses[i].burstTime;
    ...
    if (selectedProcess) {
      ...
      let createBox = `<div><p id="${selectedProcess.name}">${currentTime}</p><div>${selectedProcess.name}</div></div>`;
      document.getElementById("ganttChart").innerHTML += createBox;
