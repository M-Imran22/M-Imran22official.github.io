...
function runSchAlgo(event) {
  ...
  while (true) {
    ...
    if (remProcesses.length > 0) {
      // If there are remaining processes, select the one with the shortest remaining burst time
      const shortestRemProcess = remProcesses.reduce((min, p) =>
        p.priority < min.priority ? p : min
      );
      currProcess = shortestRemProcess;
    }
  ...
  let tableHTML = `<table>
    <caption>LNH_Priority Scheduluing Table</caption>
    <thead>
        <tr>
            <th>Process ID</th>
            <th>Arrival Time</th>
